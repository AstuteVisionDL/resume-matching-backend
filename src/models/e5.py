...
class E5(BaseJobSimilarity):
    ...
    def find_similarity(self, sent1: str, sent2: str) -> float:
        ...
        batch_dict = self.tokenizer(
            [sent1, sent2], max_length=512, padding=True, truncation=True, return_tensors="pt"
        )
    ...
    def make_predictions_e5(self, evaluation_data):
        evaluation_data["prediction"] = evaluation_data.apply(
            lambda x: self.e5_sts(x["Резюме"], x["Вакансия"]), axis=1
        )
